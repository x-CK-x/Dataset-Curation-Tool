...
class Stats_tab:
    ...
    def run_stats(self, stats_run_options, stats_load_file):
        ...
        # Example: If you need to limit rows, you could do:
        # max_display = 100
        # show_list = show_list[:max_display]
        
        if "frequency table" in stats_run_options:
            show_list = sorted(csv_table.items(), key=lambda x: x[1], reverse=True)
            dataframe = gr.update(
                visible=True,
                label=stats_run_options,
                # max_rows=size,  # <-- Removed / commented out
                value=show_list
            )
    ...
    def render_tab(self):
        with gr.Tab("Data Stats"):
            with gr.Row():
                stats_run_options = gr.Dropdown(
                    label="Run Method",
                    choices=["frequency table", "inverse freq table"]
                )
                stats_load_file = gr.Dropdown(
                    label="Meta Tag Category",
                    choices=["tags", "artist", "character", "species", "general", "meta", "rating"]
                )
                stats_run_button = gr.Button(value="Run Stats", variant='primary')

            with gr.Row():
                stats_selected_data = gr.Dataframe(
                    interactive=False,
